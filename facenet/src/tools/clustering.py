...
def face_distance(face_encodings, face_to_compare, use_muti_img=False):
    """
    ...
    if len(face_encodings) == 0:
        return np.empty((0))
    dist=1/np.linalg.norm(face_encodings - face_to_compare, axis=1)
    dist=np.sum(face_encodings*face_to_compare,axis=1)
    print(dist)
    if not use_muti_img:
        return dist
    ...
    # distances=[]
    # face_to_compare=np.asarray(face_to_compare)
    #
    # for i,e in enumerate(face_encodings):
    #     e=np.asarray(e)
    #     dist=np.mean(test2.calc_dist_mat(e,face_to_compare))
...
def _chinese_whispers(encoding_list, threshold=0.8, iterations=15):
    ...
    for idx, face_encoding_to_check in enumerate(encodings):
        ...
        distances = face_distance(compare_encodings, face_encoding_to_check,use_muti_img=False)
        encoding_edges = []
...
def test():
    ...
    import sys
    import shutil
    output=expanduser("~/dataset/video-faces/cluster")
    if not exists(output):
        makedirs(output)
    else:
        shutil.rmtree(output)
        makedirs(output)
    features_path=expanduser("~/dataset/video-faces/NAN_features")
    
    # if not exists(features_path):
    #     print("Error: %s does not exists"%features_path)
    #     exit(1)
    # names=os.listdir(features_path)
    # descriptors=[]
    # for i in range(len(names)):
    #     fea_dir=os.path.join(features_path,names[i])
    #     fea=np.load(fea_dir)
    ...
    # print(len(labels))
    facial_encodings={}
    for face in os.listdir(features_path):
        path=join(features_path, face)
        facial_encodings[path]=np.load(path)
